...
if [ ! -f "$STATE_DIR/openclaw.json" ]; then
  ...
  if [ "$CURRENT_ASK" = "on-miss" ]; then
    openclaw config set 'tools.exec.ask' 'never'
    echo "[entrypoint] Migrated tools.exec.ask: on-miss → never (headless mode)"
  fi
