...
> - `cclaw logs sentinel append --json '{"check_type":"price",...}'` writes to `sentinel_log.check_type`, which is a free-form descriptor (bare form: `price`, `liquidity`, `wallet`, `contract`, `all`, `emergency`). Used by Observer when citing log rows.
...
[cclaw expansion pending — `cclaw positions check-prices` not yet implemented; price monitoring against stop-loss/take-profit thresholds is now also handled by PriceCheckProcessor (NestJS worker). Read open positions and evaluate stops manually.]
```bash
...
[cclaw expansion pending — `cclaw positions check-liquidity` not yet implemented; liquidity monitoring is now also handled by LiquidityCheckProcessor (NestJS worker). Check liquidity snapshots via cclaw.]
```bash
...
- Save new snapshot: `cclaw liquidity add --address ... --chain ... --liquidity ...`
- **If the liquidity query fails:** log `cclaw logs sentinel append --json '{"check_type":"liquidity","status":"error","summary":"<reason>"}'` AND `cclaw alerts send --type rug_warning --agent sentinel --message "liquidity check failed — rug detection blind: <reason>"`. A failed liquidity check means you cannot detect a rug this cycle.

### 3. Wallet Check (if positions exist)
[cclaw expansion pending — `cclaw wallets check --positions` not yet implemented; dev/whale wallet monitoring is now also handled by WalletMonitorProcessor (NestJS worker). Use smart-money signals as a proxy below.]
```bash
cclaw wallets signals --since 30m --action sell --tokens-in-positions --group-by token
```
(check for dev/deployer selling via smart-money signal table)
```bash
cclaw heartbeat ping --agent sentinel --check wallet_check
```
- Check smart-money signals for dev/deployer/whale wallets selling held tokens
- If dev selling detected → write sell-all order, alert human
- If whale selling >3% of supply detected → write sell-50% order (`cclaw orders propose --json '{"action":"sell","amount":"50%","reason":"whale_selling",...}'`), alert human
- **If the query fails:** log `cclaw logs sentinel append --json '{"check_type":"wallet","status":"error","summary":"<reason>"}'` AND `cclaw alerts send --type rug_warning --agent sentinel --message "wallet check failed — dev/whale activity unknown this cycle: <reason>"`.
...
[cclaw expansion pending — `cclaw positions check-contracts` not yet implemented; contract safety monitoring is now also handled by ContractSafetyProcessor (NestJS worker). Check contract snapshots via cclaw.]
```bash
cclaw contracts list --address <ADDR> --chain <CHAIN> --limit 2
```
(compare latest vs previous snapshot for safety field changes)
```bash
...
- **If the contract query fails:** log `cclaw logs sentinel append --json '{"check_type":"contract","status":"error","summary":"<reason>"}'` AND `cclaw alerts send --type rug_warning --agent sentinel --message "contract check failed — can't detect proxy/pausable/blacklist changes: <reason>"`.
...
3. **Is a periodic summary due?** Check:
   ...
   ```
   (covers ~3h of 15-min heartbeats)
...
cclaw heartbeat ping --agent sentinel --check contract_check
```
Then log the cycle as ok with `positions_checked: 0` via `cclaw logs sentinel append` (Step 6) and reply HEARTBEAT_OK.
