...
- **If `check-wallets.js` exits non-zero or returns no JSON:** log `node scripts/db-query.js add-sentinel-log --json '{"check_type":"wallet","status":"error","summary":"<reason>"}'` AND `node scripts/send-alert.js --type rug_warning --agent sentinel --message "wallet check failed — dev/whale activity unknown this cycle: <reason>"`.
...
```
(legacy hold-back — `cclaw wallets signals` pending P5)
```bash
...
- **If `check-contract.js` exits non-zero or returns no JSON:** log `node scripts/db-query.js add-sentinel-log --json '{"check_type":"contract","status":"error","summary":"<reason>"}'` AND `node scripts/send-alert.js --type rug_warning --agent sentinel --message "contract check failed — can't detect proxy/pausable/blacklist changes: <reason>"`.
