...
Before sizing, run `cclaw system chain-config --chain <CHAIN>` and read `tiersEnabled`. Only allocate to tiers the chain supports — if a tier is missing, redistribute its target to cash.
...
All allocation percentages are per-chain. Tier ranges are bounded above by the chain's `maxMoonshotAllocation` / `maxConvictionPosition` × position-count caps from `cclaw system chain-config` — never propose past those. If `tiersEnabled` doesn't include the proposed tier, reject the trade.
...
See `AGENTS.md § Base Tier Rebalancing` for the canonical trigger/action table. Use `maxBasePosition` from `cclaw system chain-config --chain <CHAIN>` as the cap.
...
### How to Rebalance
1. Run `cclaw positions list --chain <chain>` and `cclaw system cash get --chain <chain>`
...
6. Write the orders via `cclaw orders propose` (it returns `{status, approved_by}`). If any return `status: pending`, send the rebalance proposal to the human:
   ```bash
...
After formatting the trade proposal, write the order via `cclaw orders propose`. It returns `{status, approved_by}` — branch on the returned `status`:
- `approved` → Executor will pick it up next minute. No approval call needed.
- `pending` → human approval required. `ApprovalBotService` sends Telegram approval buttons automatically.
...
**For pending orders, interactive approval buttons are sent automatically by `ApprovalBotService` (NestJS worker, ADR-0027) when an order is set to `pending` status.** Human can also approve via chat (orders skill).
...
- NEVER exceed chain-specific position limits (read `rules` from `cclaw system chain-config --chain <CHAIN>`) — regime may lower these further
...
- If `cclaw system portfolio` / `cclaw system cash get` / `cclaw system chain-config` fails: log `status: "error"` to research_log, fire `cclaw alerts send --type model_failure --agent research --message "portfolio fetch failed: <reason>"`, halt the proposal. You cannot size safely without current allocation state.
- If `cclaw orders propose` fails: log `status: "error"` and alert immediately — a proposal that was analyzed but never written to the orders table is the orphan case Observer looks for.
