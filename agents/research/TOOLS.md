...
All wallet data lives in SQLite. Always use `node scripts/db-query.js <command>`; never access the DB file directly.

### Chain discovery
- `get-chains` — list active chains.
...
- `add-to-watchlist --json '<Watchlist>'`.
- `update-watchlist --id <ID> --json '{target_entry?, reason?}'`.
- `remove-from-watchlist --id <ID>`.

### Liquidity & contract snapshots
- `get-liquidity --address <ADDR> --chain <CHAIN>`.
- `add-liquidity-snapshot --address <ADDR> --chain <CHAIN> --liquidity <N>`.
- `get-contract-snapshots --address <ADDR> --chain <CHAIN> [--limit N]`.
...
- `get-tracked-wallets [--status scored]`.
- `add-tracked-wallet --json '{address,chain,label,type}'`.
- `remove-tracked-wallet --address <ADDR> --chain <CHAIN>`.
- `propose-wallet --json '{address,chain,label,source_token}'` — fast, no API calls; consumed by background scorer.
- `get-unscored-wallets [--limit N]`.
- `update-wallet-score --address <ADDR> --chain <CHAIN> --json '{score,type,score_breakdown:{…},status:"scored"}'`.

`source_token` values: `agent`, `leaderboard`, `token_traders`, `holder_extraction`. Background scoring (`score-wallets-bg.js`, every 10 min) auto-classifies; failed wallets retry up to 3 times. See CLAUDE.md § Wallet Pipeline for the full flow.

### Smart-money signals (read-only)
Per-swap signals from `activity-wallets-bg.js` (every 30 min, 24 h retention).

- Heartbeat conviction query: `get-smart-money-signals --since 35m --action buy --group-by token --min-wallets 2`.
- Token-deep query: `get-smart-money-signals --since 1h --chain <CHAIN> --limit 50`.
