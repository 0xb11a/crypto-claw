...
8. **REQUIRED — Log to database** (one entry PER check that ran — do NOT skip this step):
   ```bash
   cclaw logs research append --json '{"check_type":"<CHECK>","tokens_scanned":<N>,"tokens_analyzed":<N>,"trades_proposed":<N>,"alerts_processed":<N>,"watchlist_hits":<N>,"summary":"<one-line summary>","status":"ok"}'
   ```
   - Write one `cclaw logs research append` call for EACH check that ran this cycle
...
- Run token discovery via the discovery skill (see TOOLS.md). [cclaw expansion pending — `cclaw market scan` not yet implemented; use the discovery skill which calls the available data APIs]
...
- For each token returned, run dedup via `cclaw analysis check`, then the full pipeline (analysis → risk → trade proposal). Treat as high-urgency discoveries.
- For deployer wallets, call `cclaw wallets propose`. Background wallet scoring (WalletScoringProcessor, every 10 min via NestJS worker) and signal production (WalletActivityProcessor, every 30 min, 24 h retention) run autonomously — heartbeat only consumes. See CLAUDE.md § Wallet Pipeline for the full flow.

### Conviction Token Scan (pipeline)
- Run conviction token discovery via the discovery skill (see TOOLS.md). [cclaw expansion pending — `cclaw market scan --sort established` not yet implemented]
- Filter through discovery skill conviction criteria (age >7d, liquidity >$100k, verified)
- Cross-reference with narrative momentum data (`cclaw system meta get --key market_regime`, or read from recent research_log)
...
- **First:** read market regime from DB: `cclaw system meta get --key market_regime`
- If regime is `bearish` or `crisis` (`baseBuyingEnabled: false`): log "Base tier buying paused — market regime: {regime}" and skip
- For each chain where `base` is in `tiersEnabled` (from `cclaw system chain-config --chain <CHAIN>`), use `maxBasePosition` from that config as the cap (see AGENTS.md § Base Tier Rebalancing)
...
- Use `cclaw positions get --id <ID>` for position data, or get current prices via the market data available in your context [cclaw expansion pending — `cclaw market price` not yet implemented]
...
- [cclaw expansion pending — `cclaw market narrative` not yet implemented; narrative momentum data is not currently surfaced via cclaw CLI]
...
- [cclaw expansion pending — `cclaw market narrative-scan` not yet implemented]
- Use recent discovery data from MEMORY.md and daily logs to identify hot narratives.
- For each hot-narrative token candidate: run dedup with `cclaw analysis check --address <ADDR> --chain <CHAIN>` (skip on `action: "skip"`), then full pipeline (analysis → risk → trade proposal)
...
- Run `cclaw positions list --chain <chain>` and `cclaw system cash get --chain <chain>`
- Check allocation vs targets, propose rebalance if needed

### Daily Summary (quick)
- `cclaw positions list --chain <chain>`, `cclaw system trade-stats --chain <chain>`, `cclaw receipts list --limit 20`
...
- Run `cclaw analysis clear-expired` to prune stale analysis cache entries

### Watchlist Entry Check (quick)
- Run `cclaw watchlist list --status watching`
...
- [cclaw expansion pending — on-chain portfolio sync is now handled by the PortfolioSyncProcessor (NestJS worker) automatically]
- Read active chains via `cclaw system chains`.
