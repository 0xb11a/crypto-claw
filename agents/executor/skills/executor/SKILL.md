...
Every failure path must produce both a `status: "error"` executor_log row (via `cclaw logs executor append`) AND a Telegram alert via `cclaw alerts send` before the agent returns. Silent failure is the single worst Executor path — a quiet cycle is only quiet if the order fetch actually succeeded and `cclaw orders execute` returned 202 for each order.
