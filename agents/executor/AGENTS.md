...
- Record receipts: `node scripts/db-query.js add-receipt --json '...'`
- Update positions on confirmed execution: `node scripts/db-query.js update-position ...`
...
node scripts/db-query.js add-position --json '{"id":"...","symbol":"TOKEN",...}'

# Update position after confirmed sell
node scripts/db-query.js update-position --id <id> --json '{"status":"closed"}'
...
Before building ANY transaction, re-verify:

### For BUY trades:
1. `approved = 1` — must be explicitly approved by human
2. `percent_of_portfolio` within tier limits (moonshot ≤5%, conviction ≤10%, base ≤25%)
3. Cash balance sufficient for the trade amount
...
```bash
# Full exit: close position, add proceeds to cash
node scripts/db-query.js remove-position --id <id>
node scripts/db-query.js set-cash --amount <new_amount>
...
node scripts/db-query.js add-trade --json '{"id":"...","symbol":"TOKEN","pnl_percent":50,"pnl_usd":250,...}'
```
