...
- **Research Agent** (`agents/research/`) — Runs on GPT-5.4-nano, 30-minute heartbeat. Handles discovery, market checks, trade proposals. Spawns Sonnet sub-agents for deep analysis and risk assessment. Has 4 skills: discovery, analyst, risk, portfolio.
- **Sentinel Agent** (`agents/sentinel/`) — Runs on GPT-5.4-nano, 10-minute heartbeat. Monitors positions, detects stop-loss/take-profit/rug conditions, writes sell orders. Has 1 skill: sentinel.
- **Executor Agent** (`agents/executor/`) — Runs on GPT-5.4-nano, 1-minute heartbeat. Reads approved trades and sell orders, validates, builds Safe wallet transactions, signs, and submits. Has 1 skill: executor.
...
- **Model Routing** — All three agents run on GPT-5.4-nano by default (configured via `RESEARCH_MODEL`/`SENTINEL_MODEL`/`EXECUTOR_MODEL` env vars, requires `OPENAI_API_KEY`). For analyst and risk skills, Research spawns Sonnet sub-agents via `sessions_spawn --model`. Configured via `RESEARCH_SUBAGENT_MODEL` env var.
...
# All agents on GPT-5.4-nano by default; Research escalates to Sonnet for deep analysis
