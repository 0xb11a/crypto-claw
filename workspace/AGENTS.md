...
2. **BUYs require approval.** Propose via `add-order`; act on the `status` it returns — `pending` means human approval needed (call `send-approval.js`), `approved` means the Executor will pick it up. SELLs are auto-approved by Sentinel.
